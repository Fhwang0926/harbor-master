...
class Modem {
  ...
  async requestAxios(options, cb) {
    ...
    try {
      ...
      else if(options.method == 'POST') { rv =  await this.instance.post(path, Object.assign({}, options.qs, { ts: new Date().getTime() })) }
      else if(options.method == 'PUT') { rv =  await this.instance.put(path, Object.assign({}, options.qs, { ts: new Date().getTime() })) }
      else if(options.method == 'DELETE') { rv =  await this.instance.delete(path, Object.assign({}, options.qs, { ts: new Date().getTime() })) }
      else {
